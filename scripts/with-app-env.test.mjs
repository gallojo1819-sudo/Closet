...
test("vite is launched via the local package, not PATH", () => {
  const run = runnable("vite", ["dev", "--host", "0.0.0.0"]);
  assert.equal(run.command, process.execPath);
  assert.match(run.args[0], /vite[/\\]bin[/\\]vite\.js$/);
  assert.deepEqual(run.args.slice(1), ["dev", "--host", "0.0.0.0"]);
});

test("non-vite commands stay as typed", () => {
  const run = runnable(process.execPath, ["-e", "0"]);
  assert.equal(run.command, process.execPath);
  assert.deepEqual(run.args, ["-e", "0"]);
});

test("local .bin is prepended to PATH", () => {
