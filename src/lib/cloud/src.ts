...
export function rewriteCloudSrcs<T extends SrcGarment>(
  ...
  return garments.map((g) => {
    ...
    let imageSrc = g.imageSrc;
    let cutoutSrc = g.cutoutSrc;
    if (o) imageSrc = cloudSrc(userId, g.id, "o");
    else if (t) imageSrc = cloudSrc(userId, g.id, "t");
    if (c) cutoutSrc = cloudSrc(userId, g.id, "c");
    else if (t) cutoutSrc = cloudSrc(userId, g.id, "t");
